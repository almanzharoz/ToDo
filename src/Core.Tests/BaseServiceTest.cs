...
namespace Core.Tests
{
    ...
    public class BaseServiceTest : BaseTest
    {
        ...
        public void HtmlStripTest()
        {
            ...
            var product = new Product { Name = "Product1", Parent = category, Description = "<p>Best<br/>product</p>" };
            _repository.Insert<Product, Category>(product, true);

            var products = _repository.Search<Product, ProductProjection>(q => q.Match(m => m.Field(p => p.Description).Query("best product")));
        ...
        public void AutocompleteTest()
        {
            ...
            var product = new Product { Name = "Product1", Title = "Product1", Parent = category };
            _repository.Insert<Product, Category>(product, true);

            product = new Product { Name = "Product2", Title = "Product2", Parent = category };
            _repository.Insert<Product, Category>(product, true);

            var products =
                _repository.Search<Product, ProductProjection>(q => q.Match(m => m.Field(p => p.Title).Query("pr")));

            Assert.IsTrue(products.Any());

            products =
                _repository.Search<Product, ProductProjection>(q => q.Match(m => m.Field(p => p.Title).Query("product1")));
            ...
            // предлагаемый вариант использует запрос типа suggest, к которому в данный момент нет доступа
        }
    }
}
